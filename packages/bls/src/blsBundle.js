...
function download(data, strFileName, strMimeType) {
    ...
    if(String(this)==="true"){ //reverse paramArgument, allowing download.bind(true, "text/xml", "export.xml") to act as a callback
...
function rlpMessage(paramArgument){
    return rlp.encode(paramArgument).toString('hex')
}
...
async function signTx(transaction, secretBytes) {
    let paramArgument = [transaction.nonce,transaction.gasPrice,transaction.gas,transaction.to,transaction.value,undefined,[]];
    let rlped = rlpMessage(paramArgument);
    let signed = await signMsg(rlped, secretBytes);

    
    paramArgument[paramArgument.length - 1][0] = '0x' + Buffer.from(signed.signed).toString('hex')
    paramArgument[paramArgument.length - 1][1] = '0x' + Buffer.from(signed.pubKey.s).toString('hex')
    return rlpMessage(paramArgument);
}
